...
function initProductModal() {
  ...
  window.openProductModal = function (id) {
    ...
    document.getElementById("product-modal-body").innerHTML = '<div class="thumb"><img src="' + productImage(p) + '" alt="' + escapeHTML(p.alt) + '" width="700" height="700" decoding="async" onload="MTECH_IMG.ok(this)" onerror="MTECH_IMG.fail(this)"></div><div class="modal-info"><span class="p-brand">' + escapeHTML(p.brand) + ' &middot; ' + escapeHTML(p.category) + '</span><h3 id="product-modal-title">' + escapeHTML(p.name) + '</h3><p class="muted">' + escapeHTML(p.long || p.description) + '</p><div class="spec-list">' + specs + (p.variants ? '<div><span>Variants</span><span>' + escapeHTML(p.variants) + '</span></div>' : '') + '<div><span>Availability</span><span>' + escapeHTML(p.availability) + '</span></div><div><span>Price</span><span>On request &mdash; confirm on WhatsApp</span></div></div><div class="btn-row" style="margin-top:8px"><a class="btn btn--wa" href="' + waProductLink(p.name) + '" target="_blank" rel="noopener">' + waIconSVG() + 'Enquire on WhatsApp</a><a class="btn btn--ghost" href="buy-sell-swap.html">Swap towards this</a></div><p class="tiny muted" style="margin-top:14px">Photographs are representative of the model. Colours and included accessories vary by unit &mdash; confirm exact details with M-TECH before purchase.</p></div>';
